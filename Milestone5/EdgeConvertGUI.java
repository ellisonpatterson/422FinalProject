...
public class EdgeConvertGUI {
   ...
   public void createDTScreen() {//create Define Tables screen
      ...
      //new code
      jmiDTHelpHelp = new JMenuItem("Help Page");
   ...
   public void createDRScreen() {
      ...
      //New code
      jmiDRHelpHelp = new JMenuItem("Help Page");
   ...
   private void getOutputClasses() {
      ...
      try {
         for (int i = 0; i < resultFiles.length; i++) {
            ...
            URLClassLoader classLoader = URLClassLoader.newInstance(new URL[] { resultFiles[i].toURI().toURL() });
            resultClass = Class.forName(resultFiles[i].getName().substring(0, resultFiles[i].getName().lastIndexOf(".")), true, classLoader);
            ...
            if (resultClass.getSuperclass().getName().equals("EdgeConvertCreateDDL")) { //only interested in classes that extend EdgeConvertCreateDDL
               if (parseFile == null && saveFile == null) {
                  ...
                  *  WHEN INVOKING getMethod("getProduceName") METHOD SEVERAL LINES BELOW
                  */
      ...
      public void actionPerformed(ActionEvent ae) {
         ...
         if ((ae.getSource() == jmiDTHelpHelp) || (ae.getSource() == jmiDRHelpHelp)) {
            try {
               ...
               URI oURL = new URI("http://rit.clickhelp.co");
               desktop.browse(oURL);
            } catch (Exception e) {
               e.printStackTrace();
            }
